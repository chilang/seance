...
fn render_stats_grid(f: &mut Frame, analysis: &SessionAnalysis, area: Rect) {
    ...
    // Row 1: Total Turns, Total Tokens, Total Cost
    ...
    render_stat_card(
        ...
        "Turn count in this thread",
        cols[0],
        COLORS.text,
    );

    // Middle column
    let row1_middle = Layout::default()
        .direction(Direction::Vertical)
        .constraints([Constraint::Percentage(50), Constraint::Percentage(50)])
        .split(cols[1]);
    ...
    render_stat_card(
        ...
        "Spend for this conversation",
        cols[2],
        COLORS.cost,
    );

    // Row 2: 3 stats across (Cache Hit Rate, Cache Write Rate, Avg Turn Cost)
    // But we have limited height, so let's adjust
